...
function addToCart(id) {
  ...
  .then(res => {
    if (!res.ok) throw new Error("Stok habis / gagal menambahkan");
    return res.text();
  })
  .then(() => loadCart())
      ...
      data.items.forEach(item => {
        cartItems.innerHTML += `
          <div class="cart-item">
            ...
            (${item.qty}) - Rp ${item.subtotal}
            <button onclick="removeItem(${item.id})">−</button>
          </div>
        `;
      });

      cartTotal.innerText = "Rp " + data.total;
    })
    .catch(() => {
      cartItems.innerHTML = "<em>Keranjang kosong</em>";
